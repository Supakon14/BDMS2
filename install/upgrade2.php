            ...
            try {
                ...
                if (!$db->tableExists($table_user_meta)) {
                    if ($db->tableExists($table_user_category)) {
                        ...
                        if ($db->indexExists($table_user_meta, 'member_id')) {
                            $db->query("ALTER TABLE `$table_user_meta` DROP INDEX `member_id`");
                        }
                        $db->query("ALTER TABLE `$table_user_meta` CHANGE `type` `name` VARCHAR(10) DEFAULT NULL");
                        $db->query("ALTER TABLE `$table_user_meta` ADD `value` VARCHAR(10) NOT NULL AFTER `id`");
                        ...
                        if ($db->fieldExists($table_user, 'department')) {
                            $db->query("INSERT INTO `$table_user_meta` SELECT `department`, 'department', `id` FROM `$table_user` WHERE `department` IS NOT NULL");
